...
class Alpha(BaseAlpha):
    def get(self, start, end):
        ...
        bonus = (
            bonus.reset_index()
            .melt(id_vars="index", value_vars=bonus.columns)
            .dropna()
        ...
        trading_days_list = [
            ...
            for i in iter_trading_days(
                TradingDay.day_delta(_start, n=-21, exchange="krx"),
                TradingDay.day_delta(end, n=21, exchange="krx"),
            )
        ]
        ...
        descriptor = cf_raw.get_df("content.handa.dataguide.descriptor.short-term-reversal.1d").rank(pct=True, axis=1)
